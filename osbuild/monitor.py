...
class JSONSeqMonitor(BaseMonitor):
    ...
    def result(self, result: Union[BuildResult, SourceResults], metadata: Optional[Dict] = None) -> None:
        ...
        if isinstance(result, BuildResult) and len(result.output) > max_output_len:  # type: ignore
            ...
            # Ignore the results of individual source items and just log
            # the overall result. The result generated by osbuild at the
            # end should have all the details in case of failures.
            log_result = SourceResults(result.source_type)
            log_result.success = result.success
            log_result.results = []
