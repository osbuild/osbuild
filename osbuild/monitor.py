...
class PrettyMonitor(BaseMonitor):  # pylint: disable=too-many-instance-attributes
    ...
    def begin(self, pipeline: osbuild.Pipeline):
        ...
        if self.total_steps > 1 and self.total_stages > 0:
            progress_desc = f"Building {
                self.total_stages} Stages (Pipeline {
    ...
    def finish(self, results: Dict):
        ...
        status_text = "[[yellow]🗸[/yellow]] Completed" if pipeline_success else "[■] Failed"

        self.console.print(f"[{status_color}]{status_text}: Pipeline {results.get('name',
        ...
        if self.completed_steps >= self.total_steps:
            ...
            status_text = "Build Completed Successfully" if results.get("success", False) else "Build Failed"
            ...
            completion_panel = Panel(
                ...
                + (f"\n[dim yellow]Errors encountered: {len(self.failed_stages)}[/dim yellow]" if self.failed_stages else ""),
                title="Build Result",
    ...
    def result(self, result: Union[BuildResult, DownloadResult]) -> None:
        ...
        if not result.success:
            ...
            self.failed_stages.append({
                ...
                'duration': duration,
                'output': result.output.strip() if result.output and result.output.strip() else None
            })
