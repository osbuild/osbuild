...
class Stage:
    ...
    def run(self, tree, runner, build_tree, store, monitor, libdir, timeout=None):
        with contextlib.ExitStack() as cm:
            ...
            binds = [os.fspath(tree) + ":/run/osbuild/tree", f"{mounts_tmpdir}:{mounts_mapped}"]
    ...
    def build_stages(self, object_store, monitor, libdir, stage_timeout=None):
        ...
        while todo:
            ...
            with build_tree.read() as build_path, tree.write() as path:
                ...
                r = stage.run(
                    path, self.runner, build_path, object_store, monitor, libdir, stage_timeout
                )
...
class Manifest:
    ...
    def add_pipeline(
        self, name: str, runner: str, build: str, source_epoch: Optional[int] = None
    ) -> Pipeline:
