...
class FdSet():
    ...
    operations is to query it for information, or steal file-descriptors from
...
class Socket():
    ...
    def __init__(self, socket, unlink):
        self._socket = socket
        self._unlink = unlink
    ...
    def close(self):
        ...
        if self._unlink is not None:
            try:
                _os.unlink(self._unlink[1], dir_fd=self._unlink[0])
            except OSError as e:
                if e.errno != _errno.ENOENT:
                    raise

            _os.close(self._unlink[0])
            self._unlink = None

    @classmethod
    def new_client(cls, connect_to: _Optional[str] = None):
        """Create Client
        ...
        try:
            socket = _socket.socket(_socket.AF_UNIX, _socket.SOCK_DGRAM)
    ...
    def new_server(cls, bind_to: str):
        ...
        try:
            ...
            socket = _socket.socket(_socket.AF_UNIX, _socket.SOCK_DGRAM)
            socket.bind(bind_to)
            unlink = _os.open(_os.path.join(".", path[0]), _os.O_CLOEXEC | _os.O_PATH)
        except:
        ...
        return cls(socket, (unlink, path[1]))
    ...
    def recv(self):
        ...
        size = 4096
        while True:
            peek = self._socket.recvmsg(size, 0, _socket.MSG_PEEK)
            if not (peek[2] & _socket.MSG_TRUNC):
                break
            size *= 2
        ...
        fds = _array.array("i")
        msg = self._socket.recvmsg(size, _socket.CMSG_LEN(253 * fds.itemsize))
        ...
        flags = msg[2]
        if flags & (_socket.MSG_TRUNC | _socket.MSG_CTRUNC):
            raise BufferError

        try:
            payload = _json.loads(msg[0])
        except _json.JSONDecodeError:
            raise BufferError

        return (payload, fdset, msg[3])

    def send(self, payload: object, *, destination: _Optional[str] = None, fds: list = []):
        """Send Message
        ...
        serialized = _json.dumps(payload).encode()
        cmsg = []
        if len(fds) > 0:
            cmsg.append((_socket.SOL_SOCKET, _socket.SCM_RIGHTS, _array.array("i", fds)))
