...
import osbuild.util.bom.model as bom_model


def bom_chksum_algorithm_from_hawkey(chksum_type: int) -> bom_model.ChecksumAlgorithm:
    """
    Convert a hawkey checksum type number to a BOM checksum algorithm.
    """
    if chksum_type == hawkey.CHKSUM_MD5:
        return bom_model.ChecksumAlgorithm.MD5
    if chksum_type == hawkey.CHKSUM_SHA1:
        return bom_model.ChecksumAlgorithm.SHA1
    if chksum_type == hawkey.CHKSUM_SHA256:
        return bom_model.ChecksumAlgorithm.SHA256
    if chksum_type == hawkey.CHKSUM_SHA384:
        return bom_model.ChecksumAlgorithm.SHA384
    if chksum_type == hawkey.CHKSUM_SHA512:
        return bom_model.ChecksumAlgorithm.SHA512
    raise ValueError(f"Unknown Hawkey checksum type: {chksum_type}")


# pylint: disable=too-many-branches
def dnf_pkgset_to_bom_pkgset(dnf_pkgset: List[dnf.package.Package]) -> List[bom_model.BasePackage]:
    """
    Convert a dnf package set to a BOM package set.
    """
    pkgs_by_name = {}
    pkgs_by_provides: Dict[str, List[bom_model.BasePackage]] = {}

    for dnf_pkg in dnf_pkgset:
        pkg = bom_model.RPMPackage(
            name=dnf_pkg.name,
        ...
        if not dnf_pkg.from_repo and dnf_pkg.remote_location():
            pkg.repository_url = dnf_pkg.remote_location().removesuffix(dnf_pkg.relativepath).strip("/")
        ...
        pkg.rpm_provides = [bom_model.RPMDependency(r.name, r.relation, r.version) for r in dnf_pkg.provides]
        pkg.rpm_requires = [bom_model.RPMDependency(r.name, r.relation, r.version) for r in dnf_pkg.requires]
        pkg.rpm_recommends = [bom_model.RPMDependency(r.name, r.relation, r.version) for r in dnf_pkg.recommends]
        pkg.rpm_suggests = [bom_model.RPMDependency(r.name, r.relation, r.version) for r in dnf_pkg.suggests]
