...
import osbuild.util.bom.spdx2 as spdx2


def spdx2_checksum_algorithm(algorithm: bom_model.ChecksumAlgorithm) -> spdx2.ChecksumAlgorithm:
    if algorithm == bom_model.ChecksumAlgorithm.SHA1:
        return spdx2.ChecksumAlgorithm.SHA1
    if algorithm == bom_model.ChecksumAlgorithm.SHA224:
        return spdx2.ChecksumAlgorithm.SHA224
    if algorithm == bom_model.ChecksumAlgorithm.SHA256:
        return spdx2.ChecksumAlgorithm.SHA256
    if algorithm == bom_model.ChecksumAlgorithm.SHA384:
        return spdx2.ChecksumAlgorithm.SHA384
    if algorithm == bom_model.ChecksumAlgorithm.SHA512:
        return spdx2.ChecksumAlgorithm.SHA512
    if algorithm == bom_model.ChecksumAlgorithm.MD5:
...
def bom_pkgset_to_spdx2_doc(pkgset: List[bom_model.BasePackage]) -> spdx2.Document:
