...
class DNF(SolverBase):
    ...
    @staticmethod
    def _dnfrepo(desc, parent_conf=None, root_dir=None):
        ...
        if "baseurl" in desc:
            repo.baseurl = [config.substitute(ucd(repo), parent_conf.substitutions)
                            for repo in desc["baseurl"]] if root_dir else desc["baseurl"]
        elif "metalink" in desc:
            repo.metalink = config.substitute(ucd(desc["metalink"]),
                                              parent_conf.substitutions) if root_dir else desc["metalink"]
        elif "mirrorlist" in desc:
            repo.mirrorlist = config.substitute(ucd(desc["mirrorlist"]),
                                                parent_conf.substitutions) if root_dir else desc["mirrorlist"]
        else:
