    ...
    def depsolve(self, arguments):
        ...
        for transaction in transactions:
            if transaction.get("module-install-specs") or transaction.get("module-enable-specs"):
                ...
                # XXX i'd really prefer to be able to get this from the transaction
                # XXX maybe that's possible, for now i take the silly route
                for module_spec in itertools.chain(
                    ...
                    # XXX this is a result of not being able to get the enabled modules
                    # XXX from the transaction, if that turns out to be possible then
                    # XXX we can get rid of these shenanigans
                    for module_package in module_packages:
                        ...
                        if any(module_nevra in package_nevras for module_nevra in module_nevras):
                            ...
                            # XXX we are probably able to skip the rest of the `module_packages`
                            # XXX here if we want since no two modules can be enabled by the same
                            # XXX name anyhow
        ...
        # XXX for each module we provide the contents for both files, this allows the
        # XXX the caller to add inline sources and copy stages we *might* want to do this
        # XXX differently instead by having specific stages and a specific format?
