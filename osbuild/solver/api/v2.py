...
def _transactions_to_disjunct_sets(transactions: List[List[Package]]) -> List[List[Package]]:
    """
    Convert a list of transactions to a list of disjunct sets of packages.

    Solver implementations always return transactions as supersets of the previous transaction results.
    This function converts the transactions to a list of disjunct sets of packages, where each transaction result contains
    only the new "to be installed" packages. Effectively making the transactions results disjunct sets of packages.
    """
    disjunct_sets: List[List[Package]] = []
    seen_packages: Set[Package] = set()
    for transaction in transactions:
        current_set = set(transaction)
        disjunct_transaction = list(current_set - seen_packages)
        seen_packages.update(disjunct_transaction)
        # NB: we sort the transaction to ensure that the transaction results are
        # kept sorted in the same way as the original transactions.
        disjunct_transaction.sort()
        disjunct_sets.append(disjunct_transaction)
    return disjunct_sets
...
def serialize_response_depsolve(solver: str, result: DepsolveResult) -> Dict[str, Any]:
    ...
    transactions = _transactions_to_disjunct_sets(result.transactions)
