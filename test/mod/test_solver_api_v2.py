...
from osbuild.solver.api.v2 import _transactions_to_disjunct_sets
...
def test_transactions_to_disjunct_sets():
    """
    Test that the _transactions_to_disjunct_sets function converts a list
    of transactions to a list of disjunct sets of packages.
    """
    transactions = [
        [TEST_PACKAGES[0]],
    ...
    disjunct_sets = _transactions_to_disjunct_sets(transactions)
    assert disjunct_sets == [
        [TEST_PACKAGES[0]],
        [TEST_PACKAGES[2]],
...
def assert_serialized_package(pkg: dict, package: Package):
    ...
    NB: This check validates that the V2 API returns all current Package model
...
def assert_serialized_repository(repo: dict, repository: Repository):
    ...
    NB: This check validates that the V2 API returns all current Repository model
    ...
    # NB: dict uses "id" but Repository uses "repo_id"
...
def test_solver_response_v2_depsolve(modules, sbom, serializer):
    # NB: each transaction contains a superset of the previous transaction.
    transactions_result = [
        [TEST_PACKAGES[0]],
        [TEST_PACKAGES[0], TEST_PACKAGES[2]],
        TEST_PACKAGES,
    ]
    # NB: the serialized transactions should be disjunct sets of packages.
    ...
    # NB: the sum of the packages in all transactions should be equal to the total number of packages
