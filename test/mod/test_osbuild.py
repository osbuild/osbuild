...
class TestDescriptions(unittest.TestCase):
    ...
    def test_on_demand(self):
        ...
        dep = manifest.add_pipeline("dep", "org.osbuild.linux", build.id)
        ...
        ul = manifest.add_pipeline("unlinked", "org.osbuild.linux", build.id)
        ...
        rootfs = manifest.add_pipeline("rootfs", "org.osbuild.inux", build.id)
        stage = rootfs.add_stage(noop, {"option": 4})

        # the main raw image artefact, depending on "dep" and
        # "rootfs"
        image = manifest.add_pipeline("image", "org.osbuild.inux", build.id)
        ...
        qcow2 = manifest.add_pipeline("qcow2", "org.osbuild.inux", build.id)
