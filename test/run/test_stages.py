...
class TestStages(test.TestBase):
    def assertTreeDiffsEqual(self, tree_diff1, tree_diff2):
        ...
        for (file1, differences1), (file2, differences2) in zip(
            ...
            for (difference1_kind, difference1_values), (
                difference2_kind,
                difference2_values,
            ) in zip(differences1.items(), differences2.items()):
                if difference1_kind != difference2_kind:
                    raise_assertion(
                        f"different difference kinds: {difference1_kind}, {difference2_kind}"
                    )
                ...
                if (
                    ...
                    raise_assertion(
                        f"before values are different: {difference1_values[0]}, {difference2_values[0]}"
                    )
                ...
                if (
                    ...
                    raise_assertion(
                        f"after values are different: {difference1_values[1]}, {difference2_values[1]}"
                    )
    ...
    def assertMetadata(self, metadata: Dict, result: Dict):
        ...
        for stageid, want in metadata.items():
            ...
            if have != want:
                diff = difflib.ndiff(
                    pprint.pformat(have).splitlines(), pprint.pformat(want).splitlines()
                )
                txt = "\n".join(diff)
    ...
    def test_dracut(self):
        ...
        with self.osbuild as osb, tempfile.TemporaryDirectory(dir="/var/tmp") as outdir:
            ...
            osb.compile_file(
                f"{base}/template.json", checkpoints=["tree"], exports=["tree"], output_dir=outdir
            )
        ...
        with self.osbuild as osb, tempfile.TemporaryDirectory(dir="/var/tmp") as outdir:
            ...
            for t in glob.glob(f"{testdir}/test_*.json"):
                ...
                manifest["pipeline"]["stages"].append(
                    {"name": "org.osbuild.selinux", "options": check["options"]}
                )
    ...
    def test_qemu(self):
        ...
        for image_name, test_data in checks.items():
            with self.osbuild as osb, tempfile.TemporaryDirectory(dir="/var/tmp") as outdir:
                osb.compile_file(
                    os.path.join(testdir, "qemu.json"), exports=[image_name], output_dir=outdir
                )
    ...
    def test_parted(self):
        ...
        with self.osbuild as osb, tempfile.TemporaryDirectory(dir="/var/tmp") as outdir:
            ...
            osb.compile_file(
                os.path.join(testdir, "parted.json"),
                checkpoints=["tree"],
                exports=["tree"],
                output_dir=outdir,
            )
    ...
    def test_sgdisk(self):
        ...
        with self.osbuild as osb, tempfile.TemporaryDirectory(dir="/var/tmp") as outdir:
            ...
            osb.compile_file(
                os.path.join(testdir, "sgdisk.json"),
                checkpoints=["tree"],
                exports=["tree"],
                output_dir=outdir,
            )
