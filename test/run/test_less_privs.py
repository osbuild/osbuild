...
def has_selinux_lsm():
    lsms_path = Path("/sys/kernel/security/lsm")
    # if unsure, assume no selinux that is always safe
    if not lsms_path.exists():
        return False
    lsms = lsms_path.read_text("utf8").split(",")
...
@pytest.mark.skipif(not shutil.which("podman"), reason="needs podman to run")
