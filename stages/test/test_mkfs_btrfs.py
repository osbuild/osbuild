...
STAGE_NAME = "org.osbuild.mkfs.btrfs"
...
@pytest.mark.parametrize("test_data, expected_err", [
    ...
    ({"uuid": "text"}, ""),
    ({"uuid": "text", "label": "osbuild"}, ""),
    ({"uuid": "text", "label": "osbuild", "metadata": "single"}, ""),
])
# This test validates only API calls using correct and incorrect queries
def test_schema_validation_mkfs_btrfs(stage_schema, test_data, expected_err):
    test_input = {
        "type": STAGE_NAME,
        "devices": {
            "device": {
...
def test_mkfs_btrfs_integration(tmp_path, stage_module):
    fake_disk_path = tmp_path / "fake.img"
    with fake_disk_path.open("w"):
        os.truncate(os.fspath(fake_disk_path), 110 * 1024 * 1024)
    devices = {
        "device": {
            "path": fake_disk_path,
        },
    }
    fake_uuid = "6d35d58f-db32-489b-99a0-08fb10e9aa29"
    fake_label = "osbuild"
    options = {
        "uuid": fake_uuid,
        "label": fake_label,
    ...
    assert re.search(f"uuid: {fake_uuid}", output), \
           f"expected UUID not found in: {output}"
    assert re.search(f"Label: \'{fake_label}\'", output), \
...
def test_mkfs_btrfs_cmdline(mock_run, stage_module, test_input, expected):
